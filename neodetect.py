...
if __name__ == "__main__":
    ...
    for i in [x for x in listdir(fldr) if 'jpg' in x]:
        debug_print(f'*** Starting detection for img {fldr}/{i}')
        child = SubElement(root, 'img')
        add_xml_subElement(child, 'name', f'{fldr}/{i}')

        img, contours = get_disc_contours(i)
        if len(contours) == 0:
        ...
        # cv2.imwrite(f'results/{i}_5.jpg', cv2.circle(img, center, radius, (0,255,0),5))
        ...
        if ratio < 80:
            ...
            result = add_xml_subElement(child, 'result', 'Healthy')
            add_xml_subElement(result, 'neovascularization', 'Neovascularization detected')
